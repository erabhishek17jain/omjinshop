...
(function ($, window, document, undefined) {
  ...
  $.fn.shopNav = function (options) {
    var defaults = {
      ...
      $underneathSpanToggle = $element
        .find('.has-dropdown')
        .children('.js-menu-toggle'),
      $megaBtn,
      $megaMenuItem,
      $megaMenuSpan,
      $megaMenuContent;
    ...
    return this.each(function () {
      ...
      var clickMegaSpan = function () {
        ...
        $megaMenuContent.removeClass('js-active');
        // add to new one index() function only works on list items , divs
        $($megaMenuContent[$(this).parent().index()]).addClass(
          'js-active'
        );
      };
      /**
      ...
      var windowCheck = function () {
        // portrait mode
        if (
          windowWidth() <= settings.mobileBreakpoint &&
          bigScreenFlag > settings.mobileBreakpoint
        ) {
          mode = 'portrait';
          bindEventsMegaMenu();
        }
        ...
        if (
          windowWidth() > settings.mobileBreakpoint &&
          smallScreenFlag <= settings.mobileBreakpoint
        ) {
          mode = 'landscape';
          bindEventsMegaMenu();
          // Flush plugin state from portrait mode
